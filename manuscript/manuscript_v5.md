...
The asymmetry ratio (pct_nterm / pct_cterm) was calculated per species and summarised per phylum/domain. This ratio is numerically unstable when pct_cterm approaches zero; for phyla with fewer than 50 C-terminal LCRs, the ratio is flagged as indicative only. A PGLS (phylogenetic generalised least-squares) regression of asymmetry ratio on phylogenetic distance from bryophytes, using the TimeTree-derived backbone for the 118 Viridiplantae species, is planned to formally test the progressive N-terminal amplification gradient claim (analysis in preparation).

### Additional planned analyses

The following analyses are in preparation and will be incorporated in the next revision:

1. **Signal peptide stratification (Bacteria):** Bacterial LCRs will be stratified by protein class (signal peptide present vs. absent) using UniProt subcellular location annotations and SignalP predictions, directly testing whether N-terminal signal sequences contribute to the observed N-terminal LCR enrichment in bacteria.

2. **Per-domain amino acid composition of terminal LCRs:** The dominant amino acid composition of terminal LCRs will be compared across Bacteria, Archaea, Viridiplantae, and Metazoa to test whether the same residues (C, E, K) are enriched at termini universally or whether prokaryote terminal LCRs reflect a different compositional class.

3. **LLPS propensity of terminal versus internal LCRs:** PLAAC (Lancaster et al. 2014) scores will be computed for terminal vs. internal LCRs in ≥5 model organisms to test whether terminal LCRs have elevated phase separation propensity — connecting the positional observation to the condensate biology framework.
...
**Viridiplantae — strongly N-terminal dominant.** Across 118 plant species, mean asymmetry ratio ≈ 3.0. Most extreme in grasses (Poaceae): Avena species (3.9–4.5), Oryza species (~3.7–4.1), Secale cereale (4.3), Triticum aestivum (3.8). Eudicots show lower but consistent N-terminal dominance (Arabidopsis thaliana: 1.93; Glycine max: 3.3). Bryophytes are near-balanced: Physcomitrium patens (moss, ~1.0), Marchantia polymorpha (~2.3). This phylogenetic gradient — lowest in mosses, intermediate in eudicots, highest in grasses — is consistent with progressive amplification of N-terminal LCR enrichment during land plant evolution. A PGLS regression formally testing this gradient is in preparation.
...
**The most informative next analysis** is signal peptide stratification: comparing N-terminal LCR fractions in bacterial proteins with vs. without predicted signal peptides (UniProt annotations; analysis in preparation). If proteins bearing signal peptides account for the majority of N-terminal terminal LCRs in bacteria, Scenario B gains strong support. If signal peptide and non-secreted proteins show equivalent N-terminal enrichment, the mechanism must be more general.
...
1. **Prokaryote statistical model:** The pooled Fisher's test does not correct for between-species clustering. A mixed-effects logistic regression is in preparation but not yet available. The 50.1% bacterial CV indicates that the pooled result is driven by a subset of LCR-rich taxa.

2. **fLPS parameter sensitivity:** The impact of parameter choice (LCR length threshold, purity cutoff, SINGLE vs. MULTI-type) on terminal enrichment estimates has not been systematically assessed. A three-parameter-combination sensitivity analysis is planned (Supplementary Table S6).
...
We report the first evidence that terminal LCR enrichment extends to prokaryotes: pooled bacteria (27.3%, 87 species) and archaea (26.2%, 21 species) show significant terminal enrichment that is not explained by protein length and increases across protein-length quartiles in the direction opposite to a length-confound artefact. This is the primary novel finding; all prior positional work was confined to eukaryotes. Whether this reflects a mechanism present since LUCA (~3.5–4 Bya) or convergent evolution in prokaryotes cannot be determined from the current data; both scenarios are equally viable pending signal peptide stratification, per-domain amino acid composition analysis, and Asgard archaea sampling.
...
**Supplementary Table S6.** [IN PREPARATION] fLPS parameter sensitivity analysis: pct_terminal and pooled significance for all 42 phyla/groups under three parameter combinations — (current: length ≥3, purity ≥70%), (stringent: length ≥6, purity ≥80%), (relaxed: length ≥3, purity ≥60%). Also includes MULTI-type LCR positional analysis.

**Supplementary Table S7.** [IN PREPARATION] Per-domain amino acid composition of terminal LCRs: dominant residue enrichment ratio (terminal / internal) per amino acid, for Bacteria, Archaea, Viridiplantae, and Metazoa separately.

**Supplementary Table S8.** [IN PREPARATION] Signal peptide stratification of bacterial terminal LCRs: pct_terminal and asymmetry ratio for proteins with vs. without predicted signal peptides (UniProt subcellular location annotations), per bacterial species with sufficient LCR counts.
...
**Supplementary Figure 8.** [IN PREPARATION] PGLS regression of N/C asymmetry ratio on phylogenetic distance from bryophytes across 118 Viridiplantae species.
